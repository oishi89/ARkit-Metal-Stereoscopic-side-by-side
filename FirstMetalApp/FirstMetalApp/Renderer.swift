...
class Renderer {
    ...
    var videoTextureY: CVMetalTexture?
    var videoTextureCbCr: CVMetalTexture?
    ...
    func update() {
        ...
        if let commandBuffer = commandQueue.makeCommandBuffer() {
            ...
            var textures = [capturedImageTextureY, capturedImageTextureCbCr, videoTextureY, videoTextureCbCr]
    ...
    func loadAssets() {
        ...
        let mesh = MDLMesh(boxWithExtent: vector3(0.095, 0.095 * 9 / 16, 0.009), segments: vector3(1, 1, 1), inwardNormals: false, geometryType: .triangles, allocator: metalAllocator)
    ...
    func updateVideoTextures() {
        ...
        videoTextureY = createTexture(fromPixelBuffer: pixelBuffer, pixelFormat:.r8Unorm, planeIndex:0)
        videoTextureCbCr = createTexture(fromPixelBuffer: pixelBuffer, pixelFormat:.rg8Unorm, planeIndex:1)
    }
    
    func drawAnchorGeometry(renderEncoder: MTLRenderCommandEncoder) {
        
        guard let textureY = videoTextureY, let textureCbCr = videoTextureCbCr, anchorInstanceCount > 0 else {
            return
        }
        ...
        renderEncoder.setFragmentTexture(CVMetalTextureGetTexture(textureY), index: Int(kVideoTextureIndexY.rawValue))
        renderEncoder.setFragmentTexture(CVMetalTextureGetTexture(textureCbCr), index: Int(kVideoTextureIndexCbCr.rawValue))
